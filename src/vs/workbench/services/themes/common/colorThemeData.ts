...
import { TokenStyle, ProbeScope, TokenStylingRule, getTokenClassificationRegistry, TokenStyleValue, TokenStyleData, TOKEN_TYPE_LANGUAGE_SEPARATOR, parseClassifierString } from 'vs/platform/theme/common/tokenClassificationRegistry';
...
export class ColorThemeData implements IWorkbenchColorTheme {
	...
	public getTokenStyleMetadata(typeWithLanguage: string, modifiers: string[], defaultLanguage: string, useDefault = true, definitions: TokenStyleDefinitions = {}): ITokenStyle | undefined {
		const [type, language] = typeWithLanguage.split(TOKEN_TYPE_LANGUAGE_SEPARATOR);
